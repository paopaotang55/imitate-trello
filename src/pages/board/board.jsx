...
import { reqGetBoards, reqAddBoard, reqDeleteBoard, reqDeleteUser, reqEditBoard } from '../../api'
...
export default class Board extends Component {
  state = {
    ...
    allBoards = () => {
      ...
      return boards && boards.map((board, index) => {
        return (
          <div key={board.id}>
            ...
            <Button shape="circle" icon="close" style={{ float: "right" }} onClick={() => this.deleteBoard( board.id)} />
            <Button style={{ float: "right" }} icon="edit" onClick={() => this.boardPlus('visibleEdit', board.boardName, board.id)} ></Button>
          </div>
        )
      })
    }
      ...
      return (
        <div className="board">
          <div className="board-div">
            <Button className="board-button" onClick={this.deleteUser}>회원탈퇴</Button>
            <Button className="board-button"><Link to="/editPassword">비밀번호 수정</Link></Button>
            <Button onClick={this.logout} className="board-button">로그아웃</Button>
          </div>
          <Card title="모든 board" style={{ width: 300 }}>
            {boards && this.allBoards()}
            <Button type="primary" style={{border: 'none'}} onClick={() => this.boardPlus('visible')}>board 추가</Button>
            <Modal
              title="board 추가"
              visible={visible}
              onOk={this.handleOk}
